...
python -c 'import importlib; modules = ["torch", "pyarrow", "datasets", "transformers", "accelerate", "huggingface_hub", "peft", "lm_eval"]; [print("%s==%s" % (name, getattr(importlib.import_module(name), "__version__", "n/a"))) for name in modules]'
