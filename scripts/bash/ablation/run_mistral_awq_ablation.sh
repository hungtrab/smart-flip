...
if [[ "$BITS" != "3" && "$BITS" != "4" ]]; then
  echo "Usage: bash scripts/bash/ablation/run_mistral_awq_ablation.sh [3|4]" >&2
  exit 2
fi

MODEL_PATH="${MODEL_PATH:-mistralai/Mistral-7B-v0.3}"
ABLATION_VARIANT="${ABLATION_VARIANT:-all}"
...
RESULTS_MODELS_ROOT="${RESULTS_MODELS_ROOT:-${RESULTS_MODELS_DIR:-./results/models/ablation/mistral_awq_b${BITS}}}"
RESULTS_EVAL_ROOT="${RESULTS_EVAL_ROOT:-${RESULTS_EVAL_DIR:-./results/eval/ablation/mistral_awq_b${BITS}}}"
...
BEST_KNEE="${BEST_KNEE:-0.01}"
BEST_MAX_FLIP="${BEST_MAX_FLIP:-0.05}"
LM_EVAL_TASKS=(
  arc_challenge
  arc_easy
  boolq
...
run_variant() {
  ...
  local run_name="awq_ablation_mistral_b${BITS}_${variant}"
  ...
  "$PYTHON_BIN" main.py quantize \
    ...
    --run-name "$run_name" \
    --wandb-tags ablation "bits:${BITS}" "variant:${variant}" model:mistral \
    "${variant_args[@]}"
}
