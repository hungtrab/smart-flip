...
RAW_EVAL_RUN_NAME="${RAW_EVAL_RUN_NAME:-gptq_raw_eval_${MODEL_SLUG}}"
BITS_VALUES=(3)
KNEE_VALUES=(0.0)
...
if [ "$USE_WANDB" = "1" ]; then
  EVAL_ARGS+=(--use-wandb --wandb-project "$WANDB_PROJECT")
...
echo "==> eval raw GPTQ model (WikiText-2, C4, LM Eval) :: ${RAW_MODEL_DIR}"
"$PYTHON_BIN" main.py float_model \
  --model-path "$RAW_MODEL_DIR" \
  --run-name "$RAW_EVAL_RUN_NAME" \
  "${EVAL_ARGS[@]}"

for bits in "${BITS_VALUES[@]}"; do
  for knee in "${KNEE_VALUES[@]}"; do
    for max_flip in "${MAX_FLIP_VALUES[@]}"; do
      run_name="${ORIGIN_METHOD}_smart_flip_${MODEL_SLUG}_from_raw_b${bits}_k${knee}_f${max_flip}"
      echo "==> smart_flip from raw :: bits=${bits} :: knee=${knee} :: max_flip=${max_flip}"
      QUANT_ARGS=(
        "${QUANT_BASE_ARGS[@]}"
